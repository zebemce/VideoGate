namespace Rtsp.Messages
{
    ...
    public class RtspMessage : RtspChunk
    {
        ...
        /// </summary>
        public RtspMessage(ILogger logger)
        {
            Data = new byte[0];
        ...
        public override void LogMessage(NLog.LogLevel aLevel, ILogger logger)
        {
            ...
            logger.Log(aLevel, "Commande : {0}", Command);
            foreach (KeyValuePair<string, string> item in _headers)
            {
                logger.Log(aLevel, "Header : {0}: {1}", item.Key, item.Value);
            }

            if (Data.Length > 0)
            {
                logger.Log(aLevel, "Data :-{0}-", ASCIIEncoding.ASCII.GetString(Data));
            }
        }
