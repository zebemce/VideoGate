...
namespace Rtsp
{
    ...
    public class H264Payload
    {
        ...
        // Constructor
        public H264Payload(ILogger logger)
        {
            _logger = logger;
        }
        ...
        {
            //_logger.Trace("RTP Data comprised of " + rtp_payloads.Count + " rtp packets");
            ...
            for (int payload_index = 0; payload_index < rtp_payloads.Count; payload_index++)
            {
                ...
                else if (nal_header_type == 24)
                {
                    ...
                    {
                        _logger.Trace("H264 Aggregate Packet processing error");
                    }
                }
                else if (nal_header_type == 25)
                {
                    _logger.Trace("Agg STAP-B not supported");
                    stap_b++;
                }
                else if (nal_header_type == 26)
                {
                    _logger.Trace("Agg MTAP16 not supported");
                    mtap16++;
                }
                else if (nal_header_type == 27)
                {
                    _logger.Trace("Agg MTAP24 not supported");
                    mtap24++;
                }
                else if (nal_header_type == 28)
                {
                    //_logger.Trace("Frag FU-A");
                    fu_a++;
                    ...
                    //_logger.Trace("Frag FU-A s=" + fu_header_s + "e=" + fu_header_e);
                ...
                {
                    //_logger.Trace("Frag FU-B not supported");
                    fu_b++;
                }
                else
                {
                    _logger.Trace("Unknown NAL header " + nal_header_type + " not supported");
                }

            }

            // Output some statistics
            _logger.Trace("Norm=" + norm + " ST-A=" + stap_a + " ST-B=" + stap_b + " M16=" + mtap16 + " M24=" + mtap24 + " FU-A=" + fu_a + " FU-B=" + fu_b);
